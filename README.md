...
## Official Argo types (in progress)

`crates/cargo-athena-argo` holds the **official** Argo types, generated by
[`kopium`] from the pinned Argo v3.6.10 CRDs in `crds/` (k8s-openapi
backed; no proto/gogo). `regenerate.sh` loosens the few intentionally
opaque CRD nodes (`steps: [][]`, recursive `inline` Template → untyped
maps) and runs kopium; the generated `.rs` is committed.

Status: **compile-only** — it builds but is *not yet wired* into
`core`/`macros` (today's emission still uses the hand subset in
`cargo-athena-api`). Next: the big-bang swap of `cargo_athena::api` to
these types. kopium is built-from-git in the flake; `k8s-openapi`'s
version feature (`v1_30`) is pinned in that crate's `Cargo.toml`.

...
Need a strict order without a real data dependency? Thread a return
value through to make the dependency explicit.

## Sub-workflow that returns a value

A `#[workflow]` with a return type bubbles its terminal task's output
up like a container:
...
`foo(bar())` runs `bar` as its own task and feeds its output into
`foo`. Shorthand for `let x = bar(); foo(x);`:
...
The default body is a DAG. Add `steps` to emit a sequential Argo
`steps:` template instead:
...
containers with an explicit dependency, bridge them with a return
value:
...
The artifact key is still a literal; the return-value just orders
the two tasks.

## Per-task hooks and resilience
...
`on_exit_if_root` is the *workflow-level* exit handler, distinct
from the per-task `.on_exit(t)` above. It runs once, when the
workflow finishes, but only for the workflow you actually submit:
...
When `pipeline` is run directly, `teardown` fires at the end
(either `argo submit --from workflowtemplate/pipeline` or
`cargo athena submit pipeline` works). When `pipeline` is
*embedded* as a sub-step of a bigger run, its own
`on_exit_if_root` stays inert; submit it directly if you want the
hook.

## Mutual exclusion across runs

At most one holder of a named mutex at a time, per cluster
namespace, so separate Workflow runs serialize against each other
(not just nested sub-workflows):
...
- **`mutexes`** is held just while the template's node is running.
  Fires anywhere the template is invoked (root or nested). Lets
  parallel tasks in one run serialize on a per-shard mutex name,
  and lets a sub-workflow self-serialize wherever it's used.

Each entry is `{ name = …, namespace = … }`; `namespace` is
optional (defaults to the workflow's own). Both fields accept the
same `"lit" + arg` injection as `image` / `env`.
...
`#[workflow]` exposes two `nodeSelector` knobs at different reach:
...
- `boundary_node_selector` covers pods whose immediate enclosing
  ...
  sub-workflows. Literal only - if you want a value that depends
  on an argument, use `node_selector_if_root`.
...
Like `host!`, declarations are collected statically and carried
through `#[fragment]` callees, so a shared helper can stamp the
same secret onto every container that uses it:
...
container. Every `host!` / artifact / `secret!` declaration it makes
is carried onto each container that transitively calls it:
...
Resources travel with the function; the calling containers don't
have to know what's inside.
